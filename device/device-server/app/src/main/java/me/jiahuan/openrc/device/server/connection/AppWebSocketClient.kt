...
import me.jiahuan.openrc.device.server.STF_DEVICE_SETTING_FILE_NAME_PATH
import me.jiahuan.openrc.device.server.connection.model.Setting
import me.jiahuan.openrc.device.server.utils.FileUtils
import okhttp3.*
...
//        postSendHeartBeat(true)
    ...
    private fun obtainRequest(): Request {
        val file = File(STF_DEVICE_SETTING_FILE_NAME_PATH)
        if (!file.exists()) {
            println("配置文件不存在")
